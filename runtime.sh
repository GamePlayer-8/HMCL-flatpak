...
cp /usr/local/bin/*.jar . || true

base_version="$(head -n 1 base_version.txt)"
package_version="${base_version}-${2:-1}"
current_date="$(date +%Y-%m-%d)"

sed -i "s/SOFTVER/${base_version}/g" *.appdata.xml
sed -i "s/SOFTVER/${base_version}/g" *.yml

sed -i "s/PACKVER/${package_version}/g" *.appdata.xml
sed -i "s/PACKVER/${package_version}/g" *.yml
...
for X in $(echo "$xml_tags" | tr ' ' '|' | tac); do
    value="$(echo "$X" | tr '|' ' ')"
    if ! grep -q "$value" *.appdata.xml; then
        sed -i "s|<releases>|<releases>\n    $value|g" *.appdata.xml
    fi
done

sed "s|<releases>|<releases>\n    $(echo -n $xml_tags)|g" *.appdata.xml

if grep -q 'version="'"$package_version"'"' *.appdata.xml; then
    updated_release="$(grep 'version="'"$package_version"'"' *.appdata.xml | sed -E "s/(<release date=\")[0-9]{4}-[0-9]{2}-[0-9]{2}(\")/\1$current_date\2/")"
    sed -i "s|$(grep 'version="'"$package_version"'"' *.appdata.xml)|${updated_release}|g" *.appdata.xml
else
    updated_release='<release date="'"$current_date"'" version="'"$package_version"'"/>"'
    sed -i "s|<releases>|<releases>\n    $updated_release|g" *.appdata.xml
fi

sed -i "s|%HMCL_VERSION%|$(cat base_version.txt)|g" org.jackhuang.hmcl.yml
